...
func main() {
    ...
    if ip == "" {
        ip, err = firstLANIPv4()
        if err != nil { log.Fatalf("detect LAN IP: %v", err) }
    }
    ...
    go func() {
        if err := srv.RunDiscovery(ctx); err != nil && ctx.Err() == nil { log.Printf("WS-Discovery stopped: %v", err) }
    }()
    go func() {
        if err := srv.Run(); err != nil && ctx.Err() == nil { log.Printf("ONVIF HTTP stopped: %v", err); cancel() }
    }()
    ...
    mgr := capture.New(cfg)
    if err := mgr.Run(ctx, ip); err != nil && ctx.Err() == nil { log.Fatalf("capture service: %v", err) }
}
